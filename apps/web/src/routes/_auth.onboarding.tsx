...
function OnboardingPage() {
  ...
  // Carrega setores quando a secretaria muda
  useEffect(() => {
    if (!selectedSec) {
      setSetores([])
      setSelectedSetor('')
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select onValueChange={setSelectedSetor} value={selectedSetor} disabled={!selectedSec || loadingSetores}>
                  <SelectTrigger className="h-10 border-slate-200 rounded-lg bg-slate-50/50 focus:ring-primary text-sm">
                    <SelectValue placeholder={loadingSetores ? 'Carregando...' : !selectedSec ? 'Selecione uma secretaria primeiro' : setores.length === 0 ? 'Nenhum setor disponível' : 'Selecione seu setor...'} />
                  </SelectTrigger>
                  <SelectContent>
                    {setores.map(setor => (
