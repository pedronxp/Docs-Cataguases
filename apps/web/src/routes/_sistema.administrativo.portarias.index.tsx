...
function PortariasListPage() {
    ...
    const toggleSelectAll = () => {
        if (selectedIds.length === portarias.length) {
            setSelectedIds([])
        } else {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-primary rounded-full" title="Entender Status" asChild>
                            <Link to="/status-documentos">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <Button asChild className="bg-primary hover:bg-primary/90 text-white font-bold h-11 px-6 shadow-md shadow-primary/10 flex-1 sm:flex-none">
            ...
            <Tabs defaultValue="geral" className="w-full" onValueChange={(val) => {
                if (val === 'assinatura') {
                    updateFilters({ status: 'APROVADA' })
                } else {
                    updateFilters({ status: undefined })
                }
                setSelectedIds([])
            }}>
                <TabsList className="bg-slate-100 p-1 h-12 w-full sm:w-auto grid grid-cols-2 sm:flex">
                    <TabsTrigger value="geral" className="data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm px-8 font-bold text-xs uppercase tracking-wider">
                        Todos Documentos
                    </TabsTrigger>
                    {canSignBatch && (
                        <TabsTrigger value="assinatura" className="data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm px-8 font-bold text-xs uppercase tracking-wider">
                            Minha Assinatura
                        </TabsTrigger>
                    )}
                </TabsList>
                            ...
                            <TableBody>
                                {portarias.length === 0 ? (
                                    ...
                                    portarias.map((doc) => (
                                        <TableRow
                                            key={doc.id}
                                            className={`hover:bg-slate-50/80 transition-colors ${doc.status === 'FALHA_PROCESSAMENTO' ? 'bg-rose-50/30' : ''} ${selectedIds.includes(doc.id) ? 'bg-primary/5' : ''}`}
                                        >
                                            ...
                                            <TableCell className="font-mono text-sm font-bold text-slate-500">
                                                {doc.numeroOficial || <span className="text-slate-300 italic">Pendente</span>}
                                            </TableCell>
                                            <TableCell>
