...
import {
  ...
  Eye, PenTool, Search, Loader2, User,
  ArrowRight, AlertTriangle, Flame, Timer,
} from 'lucide-react'
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="min-w-0">
          <h1 className="flex items-center gap-3 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">
            <Activity className="h-7 w-7 shrink-0 text-primary sm:h-8 sm:w-8" />
            <span className="min-w-0 leading-tight">Acompanhamento e Auditoria</span>
          </h1>
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto">
          <Button onClick={carregarDados} variant="outline" className="h-9 w-full font-semibold sm:w-auto" disabled={loading}>
              ...
              {gargalos.map(p => {
                ...
                return (
                  <button
                    ...
                    className="group flex w-full flex-col items-start justify-between gap-3 rounded-lg border border-rose-100 bg-white px-3 py-2 text-left transition-all hover:border-rose-300 hover:shadow-sm sm:flex-row sm:items-center"
                  >
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-[calc(100vw-1rem)] overflow-y-auto border-l-slate-200 sm:w-[540px] sm:max-w-none">
...
function MetricCard({ title, value, desc, icon, destaque = false }: {
  ...
  return (
    <Card className={`min-w-0 shadow-sm ${destaque && value > 0 ? 'border-rose-200 bg-rose-50/40' : 'border-slate-200'}`}>
      <CardContent className="flex min-w-0 items-start justify-between gap-4 p-4 sm:p-5">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase leading-snug tracking-widest text-slate-500">{title}</p>
          <p className={`text-3xl font-black tracking-tighter mt-1 ${destaque && value > 0 ? 'text-rose-700' : 'text-slate-900'}`}>{value}</p>
          <p className="mt-1 text-xs font-medium leading-snug text-slate-500">{desc}</p>
        </div>
        <div className={`shrink-0 rounded-full border p-3 ${destaque && value > 0 ? 'bg-rose-50 border-rose-100' : 'bg-slate-50 border-slate-100'}`}>{icon}</div>
      </CardContent>
    </Card>
  )
}
